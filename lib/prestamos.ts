...
export async function eliminarPagoPrestamo(movimientoId: number | string): Promise<void> {
  ...
  if (indiceRecalculo >= 0) {
    ...
    // Después de recalcular todos los movimientos, verificar el estado final del préstamo

    // Obtener los movimientos actualizados después del recálculo

    const movimientosActualizados = await obtenerMovimientosPrestamo(prestamoId)

    // Calcular el saldo final usando la misma lógica que calcularSaldoActual

    let saldoFinal = 0

    let capitalPendienteFinal = 0

    if (movimientosActualizados.length > 0) {

      const ultimoMovimiento = movimientosActualizados[movimientosActualizados.length - 1]

      if (ultimoMovimiento.saldo_pendiente !== undefined) {

        saldoFinal = ultimoMovimiento.saldo_pendiente

      } else if (ultimoMovimiento.capital_pendiente !== undefined && ultimoMovimiento.interes_pendiente !== undefined) {

        saldoFinal = redondear(ultimoMovimiento.capital_pendiente + (ultimoMovimiento.interes_pendiente || 0))

      }

      capitalPendienteFinal = ultimoMovimiento.capital_pendiente || 0

    } else {

      // Si no hay movimientos, el saldo es el monto original del préstamo

      saldoFinal = prestamo.monto

      capitalPendienteFinal = prestamo.monto

    }

    // Actualizar el estado del préstamo según el saldo final

    if (capitalPendienteFinal > 0) {

      // Hay capital pendiente, el préstamo debe estar activo
    ...
    } else if (capitalPendienteFinal === 0 && saldoFinal === 0) {

      // No hay capital ni saldo pendiente, el préstamo está pagado
    ...
    // No hay movimientos posteriores para recalcular, pero aún debemos verificar el estado final

    // Obtener los movimientos actualizados después de la eliminación
    ...
    if (capitalPendienteFinal > 0) {
      ...
      // Hay saldo pendiente, el préstamo debe estar activo
      ...
      // No hay saldo pendiente, el préstamo está pagado
