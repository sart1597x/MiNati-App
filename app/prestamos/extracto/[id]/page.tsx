...
async function calcularSaldoAFecha(prestamo: Prestamo, movimientos: PagoPrestamo[], fecha: string): Promise<number> {
  try {
...
export default function ExtractoPrestamoPage() {
  ...
  const handleFechaPagoChange = async (nuevaFecha: string) => {
    ...
    if (prestamo && movimientos.length > 0) {
      ...
      try {
        ...
        // Obtener movimientos originales para cálculo preciso

        const movimientosBD = await obtenerMovimientosPrestamo(prestamo.id!)

        const saldo = await calcularSaldoAFecha(prestamo, movimientosBD, nuevaFecha)
        ...
        // Mantener saldo actual si hay error
  ...
  const calcularSaldoALaFecha = (): number => {
    ...
    if (movimientos.length > 0 && movimientos[movimientos.length - 1].capital_pendiente <= 0) return 0
